...
<div id="page-content" class="profile2">
	...
	<ul data-toggle="ajax-tab" class="nav nav-tabs profile" role="tablist">
		...
		@endif
		@if(config('google2fa.enabled') && $employee->id == Auth::user()->id)
			<li class=""><a role="tab" data-toggle="tab" href="#tab-account-googleauth" data-target="#tab-account-googleauth"><i class="fa fa-key"></i> Google Auth</a></li>
		@endif
	</ul>
	...
	<div class="tab-content">
		...
		@endif
		@if(config('google2fa.enabled') && $employee->id == Auth::user()->id)
		<div role="tabpanel" class="tab-pane fade" id="tab-account-googleauth">
			<div class="tab-content">
				<form action="{{ url(config('laraadmin.adminRoute') . '/change_googleauth/'.$employee->id) }}" id="password-reset-form" class="general-form dashed-row white" method="post" accept-charset="utf-8">
					{{ csrf_field() }}
					<div class="panel">
						<div class="panel-default panel-heading">
							<h4>Google 2 Factor Authentication Setting</h4>
						</div>
						<div class="panel-body">
							@if (count($errors) > 0)
								<div class="alert alert-danger">
							...
							@endif
							<div class="form-group">
								<label for="password" class=" col-md-2">RQ Image</label>
								<div class=" col-md-10">
									<img src="{{ $QR_Image }}">
									<input type="hidden" name="google_code" value="{{ $google_code }}" id="google_code" class="form-control">
								</div>
							</div>
						</div>
						<div class="panel-footer">
							<button type="submit" class="btn btn-primary"><span class="fa fa-check-circle"></span> Enable Google Authentication</button>
						</div>
					</div>
				</form>
			</div>
